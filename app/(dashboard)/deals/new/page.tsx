...
export default function NewDealPage() {
  ...
  useEffect(() => {
    async function loadContacts() {
      if (!form.brand_id) { setContacts([]); return; }
      const { data: c } = await supabase.from('contacts').select('*').eq('brand_id', form.brand_id).order('name');
      if (c) setContacts(c);
    }
    loadContacts();
  }, [form.brand_id]);
  ...
  const handleCreateBrand = async () => {
    ...
    if (!user) return;
    const { data, error } = await supabase.from('brands').insert({ user_id: user.id, name: newBrandName.trim() }).select().single();
    if (data) {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          {!showNewBrand ? (
            <div className="flex gap-2">
              <select value={form.brand_id} onChange={e => setForm(p => ({ ...p, brand_id: e.target.value, contact_id: '' }))}
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select value={form.status} onChange={e => setForm(p => ({ ...p, status: e.target.value as DealStatus }))}
              className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none text-sm bg-white">
              {DEAL_STAGES.map(s => <option key={s.id} value={s.id}>{s.title}</option>)}
            </select>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="date" value={form.delivery_deadline} onChange={e => setForm(p => ({ ...p, delivery_deadline: e.target.value }))}
              className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none text-sm" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" value={form.next_action} onChange={e => setForm(p => ({ ...p, next_action: e.target.value }))}
              className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none text-sm"
        ...
        <div className="flex items-center gap-3 pt-2">
          ...
          <Link href="/pipeline" className="px-6 py-3 border border-gray-200 rounded-xl hover:bg-gray-50 text-sm text-gray-600 font-medium">
            Cancel
          </Link>
        </div>
      </form>
